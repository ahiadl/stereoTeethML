...
assert(dataset == 'kitti' or dataset == 'kitti2015' or dataset == 'mb' or dataset == 'teeth')
...
if dataset== 'teeth' then
   ...
   cmd:option('-d_brightness', 0.5)
   cmd:option('-d_contrast', 1)
elseif dataset == 'kitti' or dataset == 'kitti2015' then
   cmd:option('-hflip', 0)
   cmd:option('-vflip', 0)
   cmd:option('-rotate', 7)
...
if opt.print_args then
   ...
   print(opt.l1, 'arch1_num_layers') --number of CNN layers
   print(opt.fm, 'arch1_num_feature_maps')
   print(opt.ks, 'arch1_kernel_size')
   print(opt.l2, 'arch2_num_layers')--Number of FC layers
   ...
   print(opt.cbca_i1, 'cbca_num_iterations_1')--pre semiglobal cbca iterations
   print(opt.cbca_i2, 'cbca_num_iterations_2')--post semiglobal cbca iterations
...
print('Done initialize options according to user request')
require 'cunn'
require 'cutorch'
require 'image'
require 'libadcensus'
...
if dataset == 'teeth' then
   ...
   if opt.a == 'train_tr' or opt.a == 'train_all' or opt.a == 'test_te' or opt.a == 'test_all' or opt.a == 'submit' then
    --if opt.at == 1 then
         function load(fname)
            local X = fromfile('data.teeth/' .. fname)
            return X
         end
         X0 = load('x0.bin')
         X1 = load('x1.bin')
        ...
        -- print(metadata)
         dispnoc = fromfile('data.teeth/dispnoc.bin')
         tr = fromfile('data.teeth/tr.bin')
         te = fromfile('data.teeth/te.bin')
         ...
         X0 = fromfile('data.teeth/x0.bin')
         X1 = fromfile('data.teeth/x1.bin')
         dispnoc = fromfile('data.teeth/dispnoc.bin')
         metadata = fromfile('data.teeth/metadata.bin')
         --print(metadata)
         tr = fromfile('data.teeth/tr.bin')
         te = fromfile('data.teeth/te.bin')
         nnz_tr = fromfile('data.teeth/nnz_tr.bin')
	 --print(nnz_tr)
         nnz_te = fromfile('data.teeth/nnz_te.bin')
	 --print(nnz_te)
      --end
   end
elseif dataset == 'kitti' or dataset == 'kitti2015' then
...
function stereo_predict(x_batch, id)
   ...
   --direction: -1 = left, 1 = right (who is the refrence image)
   for _, direction in ipairs(dataset == 'mb' and mb_directions or {1, -1}) do
      ...
      if arch == 'slow' then
         if opt.use_cache then
            ...
            ------------------------------------
            ----Apply First network on both left
            ----and right images parallel  -----
            ------------------------------------
            local output = forward_free(net_te, x_batch:clone())
            clean_net(net_te)
            collectgarbage()
            if (no_negative_disp == 1) then
                vol = torch.CudaTensor(1, disp_max+1, output:size(3), output:size(4)):fill(0 / 0)
                tmpVol = torch.CudaTensor(1, disp_max+1, output:size(3), output:size(4)):fill(0 / 0)
            else
                vol = torch.CudaTensor(1, 2*disp_max+1, output:size(3), output:size(4)):fill(0 / 0)
                tmpVol = torch.CudaTensor(1, 2*disp_max+1, output:size(3), output:size(4)):fill(0 / 0)
            end
            --vol = torch.CudaTensor(1, 2*disp_max+1, output:size(3), output:size(4)):fill(0 / 0)
            --print(output:size())
            ...
            collectgarbage()
            ---------------------------------------------
            --For each disparity calc the matching cost--
            ---------------------------------------------
            for d = 1, disp_max+1 do
               --Concat the output of 2 subnetworks and push it into net_te2
               ...
               tmpVol[{1,d,{},direction == -1 and {d,-1} or {1,-d}}]:copy(net_te2.output[{1,1}])
            end
            for d = 2, disp_max+1 do
               --Concat the output of 2 subnetworks and push it into net_te2
               local l = output[{{1},{},{},{1,-d}}]
               local r = output[{{2},{},{},{d,-1}}]
               x_batch_te2:resize(2, r:size(2), r:size(3), r:size(4))
               x_batch_te2[1]:copy(l)
               x_batch_te2[2]:copy(r)
               x_batch_te2:resize(1, 2 * r:size(2), r:size(3), r:size(4))
               forward_free(net_te2, x_batch_te2)
               tmpVol[{1,d+disp_max,{},direction == -1 and {1,-d} or {d,-1}}]:copy(net_te2.output[{1,1}])
            end
            clean_net(net_te2)
            fix_border(net_te, tmpVol, direction)
      ...
      ----------------------------------------
      -----Rearrange Disparity Dimesion-------
      ----------------------------------------
      for d = 1, disp_max do
         vol[{1,d,{},{}}]:copy(tmpVol[{1,-d,{},{}}])
      end
      for d=1, disp_max+1 do
      ...
      if sm_active and opt.sm_skip ~= 'cbca' then
         ...
         local tmp_cbca = torch.CudaTensor(1, disp_max+1, vol:size(3), vol:size(4))
         if (no_negative_disp == 1) then
            tmp_cbca = torch.CudaTensor(1, disp_max+1, vol:size(3), vol:size(4))
         else
            tmp_cbca = torch.CudaTensor(1, 2*disp_max+1, vol:size(3), vol:size(4))
         end
      ...
      if sm_active and opt.sm_skip ~= 'sgm' then
         ...
         do
            ...
            if (no_negative_disp == 1) then
                ...
                vol:resize(1, 2*disp_max+1, x_batch:size(3), x_batch:size(4))
            end
...
--            vol = vol:transpose(3, 4):transpose(2, 3):clone()
      ...
      if( direction == -1) then 
         torch.DiskFile(('CBCA2-leftAhiad.bin'):format(fname), 'w'):binary():writeFloat(vol:float():storage())
      else
         torch.DiskFile(('CBCA2-rightAhiad.bin'):format(fname), 'w'):binary():writeFloat(vol:float():storage())
      end
     ----------------------------------------
     ------------Winner Takes All------------
     ----------------------------------------
      _, d = torch.min(torch.abs(vol), 2)
      --d = d:cuda():add(-(disp_max+1))
      --print(d:size())
      if( direction ==1) then
        --print(d)
      end
      for i=1,width do
          for j=1, height do
              if d[{1,1,j,i}]>0 then
                d[{1,1,j,i}] = d[{1,1,j,i}]-1
              end
              if d[{1,1,j,i}]>= disp_max+1 then
                  d[{1,1,j,i}]= (disp_max+1)-d[{1,1,j,i}]
              end
          end
      end
      --print(d)
      disp[direction == 1 and 1 or 2] = d:cuda()
   end
   collectgarbage()

   if dataset == 'kitti' or dataset == 'kitti2015' or dataset =='teeth' then
...
if opt.a == 'submit' then
   ...
   if dataset == 'kitti2015' or dataset == 'teeth' then
...
elseif opt.a == 'test_te' then
   if dataset == 'kitti' or dataset == 'kitti2015' or 'teeth' then
...
if opt.a == 'time' then
   if opt.tiny then
      ...
      disp_max = 32
   elseif dataset == 'teeth' then
      x_batch = torch.CudaTensor(2, 1, 338, 370)
   elseif dataset == 'kitti' then
...
for i=1,1,#examples do
   ...
   if opt.a == 'submit' then
      if dataset == 'kitti' or dataset == 'kitti2015' or dataset == 'teeth' then
         ...
         if dataset == 'kitti' then
            path = 'out'
         elseif dataset == 'kitti2015' or dataset == 'teeth' then
            path = 'out/disp_0'
         end
         adcensus.writePNG16(pred_img, img_height, img_width, ("%s/%.png"):format(path, id))
      elseif dataset == 'mb' then
