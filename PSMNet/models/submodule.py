...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.inplanes = 32
        # CNN
        self.firstconv = nn.Sequential(convbn(1, 32, 3, 2, 1, 1),   # conv0_1
                                       nn.ReLU(inplace=True),
                                       convbn(32, 32, 3, 1, 1, 1),  # conv0_2
                                       nn.ReLU(inplace=True),
                                       convbn(32, 32, 3, 1, 1, 1),  # conv0_3
                                       nn.ReLU(inplace=True))

        self.layer1 = self._make_layer(BasicBlock, 32, 3, 1,1,1)   # conv1_x
        self.layer2 = self._make_layer(BasicBlock, 64, 16, 2,1,1)  # conv2_x
        self.layer3 = self._make_layer(BasicBlock, 128, 3, 1,1,2)  # conv3_x
        self.layer4 = self._make_layer(BasicBlock, 128, 3, 1,1,4)  # conv4_x
    ...
    def forward(self, x):
        ...
        output_raw  = self.layer2(output)  # conv2_16
        output      = self.layer3(output_raw)
        output_skip = self.layer4(output)  # conv4_3
